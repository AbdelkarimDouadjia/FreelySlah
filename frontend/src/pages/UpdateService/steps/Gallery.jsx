...
const Gallery = () => {
  ...
  useEffect(() => {
    if (updateContext.serviceImages) {
      setImages(updateContext.serviceImages.map((url) => ({ url })));
    }
    if (updateContext.coverImage !== null) {
      setCoverImage(updateContext.coverImage);
    }
  }, [updateContext.serviceImages, updateContext.coverImage]);
  ...
  const handleImageUpload = async (e) => {
    ...
    if (validFiles.length + images.length > 20) {
      ...
      const newImages = [...images, ...validFiles.map((file) => ({ file }))];
      setImages(newImages);
      ...
      try {
        ...
        updateContext.setServiceImages(updatedImages);
  ...
  const handleRemoveImage = (index) => {
    ...
    setImages(newImages.map((img) => ({ url: img.url })));
    updateContext.setServiceImages(newImages.map((img) => img.url));
    if (index === coverImage) {
      setCoverImage(null);
