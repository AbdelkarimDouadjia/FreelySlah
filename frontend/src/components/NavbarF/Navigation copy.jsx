      ...
      >
        {navigation.map((item, i) => (
          ...
          >
            ...
            {item.children && (
              <ul
                className={`sub-menu  bg-white rounded-[12px] shadow-sdp py-[15px] px-0 z-[9999] w-[260px] hidden list-none clear-both m-0 max-[768px]:static absolute  ${
                  item.id == 1 ? "home-menu" : ""
                } `}
              >
                {item.children?.map((item2, i2) => (
                  <li
                    key={i2}
                    className={`menu-active border-l-2 border-l-transparent py-0 px-[30px]  transition-all duration-500 ease-linear relative list-none hover:bg-[#F0EFEC] hover:border-l-[#5BBB7B]`}
                  >
